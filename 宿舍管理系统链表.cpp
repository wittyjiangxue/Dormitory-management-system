# include<stdio.h>//动态 可以扩容 加入数据库可持久化的存储内容 
# include<string.h>
# include<stdlib.h>
# include<windows.h>
# define F "学生宿舍信息.txt" //文件路径
...
Student *input(int n)                  
{
	...
	/*l=(Stu *)malloc(sizeof(Stu));//分配指针给一个实体 分配空间 
	printf("\t\t宿舍楼号:");
	scanf("%d",&l->lh);
	printf("\t\t房间号:");                
	scanf("%d",&l->roomnum);
...
int sort_select(Stu *head,int n)
{
    ...
    switch(n)//功能选择 
	{
		case 1:
			system("cls");
			for(p=head;p!=NULL;p=p->next)//嵌套循环  选择排序 确定第i个是最小的 
			{
			    for(q=p->next;q!=NULL;q=q->next)//拿第i个最小的跟后面的进行比较 
			    {
			        if(p->lh>q->lh)//三变量法 
			        {
			        	lh=q->lh;
